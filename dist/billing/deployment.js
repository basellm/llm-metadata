...
import { buildModelBillingExpr } from './expr.js';
export const QUOTA_DISPLAY_TYPES = ['USD', 'CNY', 'CUSTOM'];
/** new-api 出厂默认（USD2RMB / USDExchangeRate 均为 7.3） */
export const NEW_API_DEFAULTS = {
 ...
 */
export function defaultDeployment(exchangeRates) {
    const cny = exchangeRates.CNY;
    return {
        ...NEW_API_DEFAULTS,
        usdExchangeRate: typeof cny === 'number' && Number.isFinite(cny) && cny > 0
            ? cny
...
export function sameDeployment(a, b) {
    return (a.quotaDisplayType === b.quotaDisplayType &&
...
export function sanitizeDeployment(value, defaults = NEW_API_DEFAULTS) {
    const raw = (typeof value === 'object' && value !== null ? value : {});
    const symbol = typeof raw.customCurrencySymbol === 'string' ? raw.customCurrencySymbol.trim() : '';
    const settlement = raw.usdSettlementRate;
    return {
        quotaDisplayType: QUOTA_DISPLAY_TYPES.includes(raw.quotaDisplayType)
        ...
        customCurrencyExchangeRate: positive(raw.customCurrencyExchangeRate, defaults.customCurrencyExchangeRate),
        usdSettlementRate: typeof settlement === 'number' && Number.isFinite(settlement) && settlement >= 0
            ? settlement
...
export function displayRate(deployment) {
...
export function displaySymbol(deployment) {
    switch (deployment.quotaDisplayType) {
        case 'CNY':
            return '¥';
        case 'CUSTOM':
 ...
 */
export function planeRates(deployment, fallbackFx = {}) {
    const perRealUsd = quotaUsdPerRealUsd(deployment);
    const unitsPerQuotaUsd = (unitsPerRealUsd) => unitsPerRealUsd / perRealUsd;
    const rates = {};
    for (const [code, fx] of Object.entries(fallbackFx)) {
        if (Number.isFinite(fx) && fx > 0)
            rates[code] = unitsPerQuotaUsd(fx);
    }
...
export function selectPriceSheet(cost, preferred) {
    if (!cost)
        return undefined;
    const option = preferred !== (cost.currency ?? 'USD') ? cost.currency_options?.[preferred] : undefined;
    return option ? { ...option, currency: preferred } : cost;
}
/** 按部署配置为单个模型生成 new-api 表达式 */
export function buildDeploymentBillingExpr(cost, options, deployment, fallbackFx) {
