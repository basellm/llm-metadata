(function () {
  ...
  const getDefaultApiBase = () => {
    ...
    if (hostname === 'basellm.github.io') {
      ...
      // For local development or other domains, use relative path
      return './api';
    }
  };

  const API_BASE = (root.getAttribute('data-api') || getDefaultApiBase()).replace(/\/$/, '');
  ...
  const setOpenIssueEnabled = (enabled) => {
    const btn = $('open-issue');
  ...
  function parseAndValidateBatch() {
    ...
    try {
      ...
      if (errs.length)
        return { valid: false, models, error: `${t('batchValidateFailed')}: ${errs[0]}` };
  ...
  function validateSingleRequired() {
    ...
    const mid = value('id');
    if (!pid) {
      setStatus(t('providerRequired'), true);
      return false;
    }
    if (!mid) {
  ...
  function validateBatchModels(models) {
    ...
    (models || []).forEach((m, i) => {
      if (!m || typeof m !== 'object') {
        errors.push(`${i + 1}. ${t('batchInvalidItem')}`);
        return;
      }
  ...
  function buildPayload() {
    ...
    return prune({
      ...
      cost: {
        ...
        cache_read: num('cost-cache-read'),
        cache_write: num('cost-cache-write'),
      },
    });
  }
  ...
  const buildModelUrls = (providerId, variants) =>
    buildApiUrls(providerId, variants, [
      `${API_BASE}/models/{provider}/{model}.json`,
      `${API_BASE}/i18n/${lang}/models/{provider}/{model}.json`,
    ]);

  const buildI18nModelUrls = (providerId, variants, locale) =>
    buildApiUrls(providerId, variants, [
  ...
  async function loadModelDetail(providerId, modelId) {
    try {
      ...
      const data = await fetchFirstOk(candidateUrls).catch(() =>
        findModelInAll(providerId, variants),
      );
      ...
      setValue(
        'last-updated',
        pick(data, ['last_updated', 'lastUpdated', 'updated_at', 'updatedAt']),
      );
      ...
      const capabilities = [
        ...
        [
          'cap-tools',
          ['tool_call', 'tools', 'toolCalling', 'capabilities.tools', 'features.tools'],
        ],
  ...
  const buildModelListForBatch = (models) => {
    return models
      .map((m, i) => {
        ...
        const name =
          m.i18n?.name?.[lang] || m.i18n?.name?.en
            ? ` - ${m.i18n.name[lang] || m.i18n.name.en}`
            : '';
  ...
  function buildIssue() {
    ...
    if (mode === 'batch' && Array.isArray(p)) {
      ...
      const requestType =
        single.action === 'update' ? t('updateModelRequest') : t('createModelRequest');
      ...
      body = buildIssueBody([
        ...
        single.i18n?.description?.en
          ? `- **${t('description')}**: ${single.i18n.description.en}`
          : '',
        `- **${t('actionType')}**: ${action}`,
        '',
        `## 🔧 ${t('techInfo')}`,
    ...
    const template = [
      ...
      },
      {
        ...
        i18n: {
          ...
          description: {
            en: 'Advanced conversational AI model for natural language processing.',
            zh: '用于自然语言处理的先进对话AI模型。',
            ja: '自然言語処理のための高度な対話AIモデル。',
          },
        },
