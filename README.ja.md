...
高スループットに親和的な静的インターフェース: 変更時のみ再ビルドし、GitHub Pages から静的 JSON を配信します。ライブサイトにはミニマルな価格ブラウザ（`web/`、Vite + React + Tailwind + shadcn/ui + Magic UI）が含まれ、静的 API から直接プロバイダーごとのモデル価格テーブルを表示します。ライト/ダークテーマ（既定でシステムに追従）と英語/中国語/日本語の UI に対応しています。
