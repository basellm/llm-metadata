...
面向高并发的静态接口：仅在源数据或配置变化时重建，其余时间通过 GitHub Pages 提供静态 JSON。线上站点内置一个极简价格浏览器（`web/`，Vite + React + Tailwind + shadcn/ui + Magic UI），直接读取静态 API 渲染各供应商的模型价格表，支持浅色/深色主题（默认跟随系统）与中/英/日三语界面。
