...
import { catalogPaths, loadCatalog, type Catalog, type CatalogPaths } from './services/catalog.js';
...
class Builder {
  ...
  private readonly indexBuilder = new IndexBuilder();
  ...
  private writeVoApi(
    ...
    for (const [name, data] of [
      ...
      if (
        writeJSONIfChanged(join(outDir, name), { success: true, message: '', data }, { dryRun })
      ) {
        changes++;
      }
    }
  ...
  async build(config: BuildConfig): Promise<void> {
    ...
    const localized = new Map<string, NormalizedData>();
    for (const locale of locales) {
      ...
      if (result.untranslated.length > 0) {
        warnings.push(
          `i18n: ${result.untranslated.length} model description(s) have no ${locale} translation (run "npm run translate")`,
        );
      }
    }
    ...
    if (writeJSONIfChanged(join(this.API_DIR, 'index.json'), indexes, { dryRun })) changes++;
    ...
    for (const [locale, dataset] of localized) {
      ...
      if (writeJSONIfChanged(join(outDir, 'all.json'), dataset.providers, { dryRun })) changes++;
      if (writeJSONIfChanged(join(outDir, 'index.json'), indexesLoc, { dryRun })) changes++;
      if (
        writeJSONIfChanged(
          join(outDir, 'providers.json'),
          this.indexBuilder.buildProvidersOutput(indexesLoc),
          { dryRun },
        )
      ) {
        changes++;
    ...
    const tagMapFor = (locale: string): Record<string, string> => ({
    ...
    for (const [locale, dataset] of localized) {
      changes += this.writeNewApiSync(
        join(i18nDir, locale, 'newapi'),
        newApiBuilder,
        dataset,
        tagMapFor(locale),
        dryRun,
    ...
    if (
      writeJSONIfChanged(join(newapiDir, 'ratio_config-v1-base.json'), priceConfig.config, {
        dryRun,
      })
    ) {
      changes++;
    }
    {
      ...
      const keepProviderDirs = new Set(
        Object.keys(allModelsData.providers).map(sanitizeFileSegment),
      );
      ...
      for (const providerId of Object.keys(allModelsData.providers)) {
        ...
        if (
          writeJSONIfChanged(
            join(outDir, 'ratio_config-v1-base.json'),
            providerPriceConfig.config,
            { dryRun },
          )
        ) {
          changes++;
