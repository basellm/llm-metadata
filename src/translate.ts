...
async function translateBatch(
  ...
  if (!Array.isArray(parsed.translations)) throw new Error('missing "translations" array');

  const expected = new Map(items.map((item) => [item.id, item.text]));
  const result = new Map<number, string>();
  for (const entry of parsed.translations as Array<{ id?: unknown; text?: unknown }>) {
    const text = typeof entry.text === 'string' ? entry.text.trim() : '';
    if (typeof entry.id !== 'number' || !expected.has(entry.id) || !text) continue;
  ...
  for (let start = 0; start < sources.length; start += BATCH_SIZE) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        for (const item of items) translated.set(item.text, batch.get(item.id)!);
        break;
      } catch (error) {
        const reason = error instanceof Error ? error.message : String(error);
        console.warn(
          `[${language.locale}] batch ${start / BATCH_SIZE + 1} attempt ${attempt} failed: ${reason}`,
        );
      }
    }
...
async function main(): Promise<void> {
  ...
  for (const provider of Object.values(catalog.data.providers)) {
    for (const model of Object.values(provider.models || {})) {
      if (model.description) liveSources.add(model.description);
    }
  }

  let failed = false;
  for (const language of targets) {
    const { untranslated } = catalog.processor.localizeNormalizedData(
      catalog.data,
      catalog.overrides,
      language.locale,
    );
    ...
    console.log(
      `[${language.locale}] ${missing.length} description(s) to translate, ${stale.length} stale entr${stale.length === 1 ? 'y' : 'ies'} to prune`,
    );
    if (dryRun) continue;

    const fresh = missing.length > 0 ? await translateAll(config, language, missing) : new Map();
    if (fresh.size < missing.length) failed = true;

    const next: Record<string, string> = {};
    for (const [source, text] of Object.entries(memory)) {
      if (liveSources.has(source)) next[source] = text;
    }
    for (const [source, text] of fresh) next[source] = text;
