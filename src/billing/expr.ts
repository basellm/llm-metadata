...
import { COST_FAMILIES, type CostFamily } from './cost-families.js';
import type {
  BillingExprOptions,
  CostFamilyCells,
  CostSchedule,
 ...
 */
export function buildModelBillingExpr(
  cost: ModelCost | undefined,
  options: BillingExprOptions,
  rates: PlaneRates,
): ModelBillingExprResult {
  const plane = normalizeCostToPlane(cost, rates);
  if (plane.unknownCurrency) {
    return { expr: null, warnings: [], unknownCurrency: plane.unknownCurrency };
  }
  if (!plane.cost) return { expr: null, warnings: [] };
  return buildBillingExpr(plane.cost, options);
}
