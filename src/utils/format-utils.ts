...
function formatTokensToKM(tokens?: number): string | null {
...
/** 能力标志 → 标签键 */
const CAPABILITY_TAGS: ReadonlyArray<[keyof Model, string]> = [
  ['reasoning', 'reasoning'],
  ['tool_call', 'tools'],
  ['attachment', 'files'],
...
/** 模态 → 标签键 */
const MODALITY_TAGS: ReadonlyArray<[string, string]> = [
  ['image', 'vision'],
  ['audio', 'audio'],
];

/** 归一化显式标签：去空白、去空项 */
export function explicitTags(model: Model): string[] {
  return (model.tags ?? []).map((tag) => String(tag).trim()).filter(Boolean);
}

/** 构建 NewAPI 模型标签：显式标签 + 能力 + 模态 + 生命周期状态 + 上下文窗口（经 map 本地化） */
export function buildModelTags(model: Model, map?: Record<string, string>): string[] {
  const translate = (key: string) => map?.[key] ?? key;
  const tags = new Set<string>(explicitTags(model).map(translate));

  for (const [capability, tag] of CAPABILITY_TAGS) {
    if (model[capability]) tags.add(translate(tag));
  }

  const modalities = [...(model.modalities?.input ?? []), ...(model.modalities?.output ?? [])];
  for (const [modality, tag] of MODALITY_TAGS) {
    if (modalities.includes(modality)) tags.add(translate(tag));
  }

  if (model.status) tags.add(translate(model.status));

  const context = formatTokensToKM(model.limit?.context);
  if (context) tags.add(translate(context));
